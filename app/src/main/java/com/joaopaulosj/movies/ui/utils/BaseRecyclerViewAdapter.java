...
public abstract class BaseRecyclerViewAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public int getItemViewType(int position) {
        if (isLoading() && isPlaceHolderPosition(position)) {
            ...
            return TYPE_EMPTY;
        } else if (isWithHeader() && isPositionHeader(position)) {
            return TYPE_HEADER;
        ...
        } else if (isWithFooter() && isPositionFooter(position)) {
            return TYPE_FOOTER;
        } else {
            return TYPE_ITEM;
        }
    ...
    public boolean isPlaceHolderPosition(int position) {
        return (isWithHeader() && mAlwaysShowHeader && position == 1) ||
                (!isWithHeader() && position == 0) ||
    ...
    public int getItemCount() {
        if (getDisplayableItemsCount() == 0 || mIsLoading || mIsError) {
            ...
            return getDisplayableItemsCount() + (isWithFooter() ? FOOTER_SIZE : 0) + (isWithHeader() ? HEADER_SIZE : 0) + (isPaginated() && !isListEndReached() ? PLACE_HOLDER_SIZE : 0);
        }
    }
    ...
    private int getRealPosition(int position) {
        return position - (isWithHeader() ? HEADER_SIZE : 0);
    }
    ...
    public final boolean isWithFooter() {
        try {
